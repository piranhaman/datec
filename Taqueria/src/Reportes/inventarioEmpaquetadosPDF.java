 ...
 * @author el dios
 */
public class inventarioEmpaquetadosPDF {
    ...
    public inventarioEmpaquetadosPDF(String titulo, String nomPDF) throws SQLException {
    ...
    private void agregarTabla(Paragraph parrafo) throws BadElementException {
        ...
        float anchosFilas[] = {.5f, 2f, 1f, 1f, 1f};
        PdfPTable tabla = new PdfPTable(anchosFilas);
        
        
        String rotulosColumnas[] = {"ID", "Nombre", "Cantidad", "Costo Compra $", "Precio Venta $"};
...
primary key(id));
        */
            strConsultaSQL = "select * from productosEmpaquetados";
            //ejecutar consulta
            ...
            while (rs.next()) {
                ...
                tabla.addCell(cell);
                cell = new PdfPCell(new Paragraph("$ "+String.format("%.2f",rs.getFloat("costo")), fuente9));
                tabla.addCell(cell);
                cell = new PdfPCell(new Paragraph("$ "+String.format("%.2f",rs.getFloat("precio")), fuente9));
                tabla.addCell(cell);
            }
            parrafo.add(tabla);
